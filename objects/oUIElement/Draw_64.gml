...
var editor = flexpanel_node_get_data(
	flexpanel_node_get_child(oUI.ui.root, "panel_side_ignore")
);
...
editor_area = [
	editor.inst.x,
	editor.inst.y,
	editor.inst.x + editor.inst.width,
	editor.inst.y + editor.inst.height
];
...
if (data.owner == oUI.ui) {
	...
	if (edit_node != undefined) {
		...
		if (panel != undefined && !string_contains(name, "ignore")) {
	...
	if (
		...
		&& string_contains(name, "draggable")
	) {
		dragging = true;
		if (type == "panel" && !can_drag_panel) {
			dragging = false;
		}
	}
...
if (editable) {
	if (mouse_in_area_gui([x, y, x + width, y + height])) {
		if (global.edit_node != name && type == "panel") {
	...
	if (resizing != noone && device_mouse_check_button_released(0, mb_left)) {
	...
	var _color = "[c_blue]";
	if (global.edit_node == name || oUI.editing == name || oUI.last_edit == name) {
		_color = "[c_red]";
	...
	if (!string_contains(name, "grid") || keyboard_check(ord("Z"))) {
		...
		if (
			type == "panel"
			&& keyboard_check(vk_f2) || oUI.editing == name || oUI.last_edit == name
		) {
			if (_color == "[c_blue]") {
				_color = "[c_purple]";
			}
