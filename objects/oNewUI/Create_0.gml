...
addbtn = new button("New");
addbtn
	.set_function(method(self, function() {
		oEditableUI.ui.dispose();
		oEditableUI.ui = new window(AirLib.templates[$ tagsdrop.text], true);
		oEditableUI.ui.edit_mode(true);
		instance_destroy();
	}));
delbtn = new button("Cancel");
delbtn
	.set_function(method(self, function() {
		instance_destroy();
	}));
for (var tags = struct_get_names(AirLib.templates), i = 0; i < array_length(tags); i++) {
