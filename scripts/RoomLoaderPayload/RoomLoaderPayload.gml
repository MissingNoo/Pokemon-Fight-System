...
function RoomLoaderPayload(_room) constructor {
	...
	static __GetTargetDepth = function(_lod, _methodName) {
		...
		if (is_real(_lod)) {
			...
			var _message = $"Could not shift layer depths for Layer Or Depth <{
				_lod
	...
	static GetLayer = function(_name) {
		__RoomLoaderCatchString(
			__messagePrefix,
			"GetLayer",
			_name,
			"get a Layer ID from the",
			"name"
		);
	...
	static Cleanup = function(_destroyLayers = true) {
		...
		if (__cleanedUp) {
			__RoomLoaderLogMethod(
				__messagePrefix,
				_methodName,
				$"data for \<{room_get_name(__room)}\> is already cleaned up"
			);
			return;
		}
