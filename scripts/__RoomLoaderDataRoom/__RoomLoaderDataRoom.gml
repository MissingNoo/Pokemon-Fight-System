...
function __RoomLoaderDataRoom(_room) constructor {
	...
	static __Init = function() {
		static _MapInstanceData = function(_dataIn) {
			...
			var _dataOut = {
				...
				preCreate: {},
				creationCode: ((_dataIn.creation_code != -1)
					? _dataIn.creation_code
					: __RoomLoaderNoop),
			};
	...
	static __Load = function(
		...
		if (__ROOMLOADER_NOTRANSFORM) {
			...
			var _i = 0;
			repeat (array_length(__layersPool)) {
	...
	static __TakeScreenshot = function(
		_pLeft01,
		_pTop01,
		_pWidth01,
		_pHeight01,
		...
		var _scaled = (_xScale != 1) || (_yScale != 1);
