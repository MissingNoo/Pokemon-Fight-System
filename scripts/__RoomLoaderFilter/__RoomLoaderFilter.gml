	...
	static __Add = function(_layerName) {
		...
		if (__GetIndex(_layerName) != -1) {
			return __RoomLoaderLogMethod(
				__messagePrefix,
				_methodName,
				$"Layer \"{_layerName}\" is already {__name}ed"
			);
		}
	...
	static __Remove = function(_layerName) {
		...
		if (_index == -1) {
			return __RoomLoaderLogMethod(
				__messagePrefix,
				_methodName,
				$"Layer \"{_layerName}\" is not {__name}ed"
			);
		}
		...
		return __RoomLoaderLogMethod(
			__messagePrefix,
			_methodName,
			$"Removed layer \"{_layerName}\" from {__name}"
		);
	};
