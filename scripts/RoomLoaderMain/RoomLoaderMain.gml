...
function RoomLoader() {
	...
	static __LayerFailedFilters = function(_name) {
		var _match =
			__layerWhitelist.__check(_name) && (!__layerBlacklist.__check(_name));
		return !_match;
	};
	...
	static __GetLoadData = function(_room, _methodName, _nonRoomMessage, _noDataMessage) {
		...
		var _message = $"Could not find the data for room {
			_roomName
		}.\nMake sure to initialize data for your rooms before trying to {
			_noDataMessage
		}";
		__RoomLoaderErrorMethod(__messagePrefix, _methodName, _message);
	};

	static __Screenshot = function(
		_room,
		_left,
		_top,
		_width,
		...
		var _screenshot = _data.__TakeScreenshot(
			_left,
			_top,
			_width,
			_height,
	...
	static DataInitPrefix = function(_prefix) {
		...
		if (_n == 0) {
			__RoomLoaderLogMethod(
				__messagePrefix,
				_methodName,
				$"Could not find any rooms starting with \"{_prefix}\""
			);
			return _rooms;
		}
	...
	static DataInitTag = function(_tag) {
		...
		if (_n == 0) {
			__RoomLoaderLogMethod(
				__messagePrefix,
				_methodName,
				$"Could not find any rooms with the \"{_tag}\" tag assigned"
			);
			return _rooms;
		}
	...
	static DataInitAll = function(_blacklist = []) {
		...
		if (array_length(_blacklist) > 0) {
			var _i = 0;
			repeat (array_length(__allRooms)) {
				var _room = __allRooms[_i];
				if (!array_contains(_blacklist, _room)) {
	...
	static DataIsInitialized = function(_room) {
		__RoomLoaderCatchNonRoom(
			__messagePrefix,
			"DataIsInitialized",
			_room,
			$"check whether data is initialized for"
		);
	...
	static DataGetInstances = function(_room) {
		...
		var _data = __GetLoadData(
			_room,
			_methodName,
			"get instances data for",
			"get their instances data"
		);
	...
	static LoadInstances = function(
		...
		} else if (is_string(_lod) || is_handle(_lod)) {
			_func = instance_create_layer;
		} else {
			var _message = $"Could not load instances at layer or depth <{
				_lod
	...
	/// @context RoomLoader
	static LoadTilemap = function(
		_room,
		_x,
		_y,
		_sourceLayerName,
		_targetLayer = _sourceLayerName,
		_xOrigin = __xOrigin,
		_yOrigin = __yOrigin,
		_mirror = (__xScale == -1),
		_flip = (__yScale == -1),
		...
		if ((!_mirror) && (!_flip) && (_angle == 0)) {
			...
			var _tilemap = _tilemapData.__CreateTilemapTransformed(
				_targetLayer,
				_x,
				_y,
				_xScale,
	...
	/// Returns a Sprite ID.
	/// @context RoomLoader
	static ScreenshotPart = function(
		_room,
		_left,
		_top,
		_width,
