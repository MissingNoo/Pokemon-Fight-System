...
function __RoomLoaderGetLayer(_data) {
	if (ROOMLOADER_MERGE_LAYERS && layer_exists(_data.name)) {
...
function __RoomLoaderLogMethodTimed(_prefix, _methodName, _message, _room) {
	...
	_message = $"{_message} <{room_get_name(_room)}> in {
		__ROOMLOADER_BENCH_END
	} milliseconds";
...
function __RoomLoaderCatchNonRoom(_prefix, _methodName, _room, _message) {
	...
	if ((_type == "ref") && room_exists(_room)) {
		return;
	}
...
function __RoomLoaderCatchArgument(
	_prefix,
	_methodName,
	_value,
	_checker,
...
function __RoomLoaderCatchString(
	_prefix,
	_methodName,
	_value,
	_preMessage = undefined,
	_postMessage = undefined
) {
	__RoomLoaderCatchArgument(
		_prefix,
		_methodName,
		_value,
		is_string,
...
function __RoomLoaderCatchArray(
	_prefix,
	_methodName,
	_value,
	_preMessage = undefined,
