...
function __RoomLoaderDataLayerAsset(_layerData, _data) : __RoomLoaderDataLayerParent(
	_layerData
) constructor {
	// shared
	static __flag =
		ROOMLOADER_FLAG.SPRITES
		| ROOMLOADER_FLAG.SEQUENCES
		| ROOMLOADER_FLAG.TEXTS;

	static __Draw = function(_flags) {
		if (!__layerData.visible) {
			return;
		}
		if (__HasFailedFilters()) {
			return;
		}

		var _i = 0;
		repeat (array_length(__data)) {
			with (__data[_i]) {
		...
		repeat (array_length(__data)) {
			...
			switch (_data.type) {
				case layerelementtype_sprite:
					_constructor = __RoomLoaderDataLayerAssetSprite;
					break;
				case layerelementtype_sequence:
					_constructor = __RoomLoaderDataLayerAssetSequence;
					break;
				case layerelementtype_text:
					_constructor = __RoomLoaderDataLayerAssetText;
					break;
			}
