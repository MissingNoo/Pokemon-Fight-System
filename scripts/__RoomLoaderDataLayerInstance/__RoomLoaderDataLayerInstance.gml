...
function __RoomLoaderDataLayerInstance(
	_layerData,
	_instancesData
	...
	static __OnLoad = function(_layer, _x1, _y1) {
		...
		var _i = 0;
		repeat (array_length(__instancesPool)) {
	...
	static __OnDraw = function() {
		...
		repeat (array_length(__instancesPool)) {
			with (__instancesPool[_i]) {
				...
				draw_sprite_ext(
					sprite,
					preCreate.image_index,
					x,
					y,
					preCreate.image_xscale,
					preCreate.image_yscale,
					preCreate.image_angle,
					preCreate.image_blend,
					preCreate.image_alpha
				);
			}
			_i++;
		}
