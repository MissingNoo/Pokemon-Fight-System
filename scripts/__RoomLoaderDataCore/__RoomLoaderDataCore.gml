...
function __RoomLoaderDataCore() constructor {
	static __messagePrefix = RoomLoader.__messagePrefix, __pool = {};
	...
	static __Add = function(_room, _methodName) {
		...
		if (struct_exists(__pool, _roomName)) {
			__RoomLoaderLogMethod(
				__messagePrefix,
				_methodName,
				$"Data for <{_roomName}> is already initialized"
			);
			return;
		}
	...
	static __Remove = function(_room, _methodName) {
		...
		if (!struct_exists(__pool, _roomName)) {
			__RoomLoaderLogMethod(
				__messagePrefix,
				_methodName,
				$"Data for <{_roomName}> doesn't exist, there's nothing to remove"
			);
			return;
		}
